	...
	class Themify_Team {
		...
		function progress_bar( $atts = array() ) {
			...
			return sprintf(
				'<div class="progress-bar">
					...
					<span class="progress-bar-bg" data-percent="%s" style="background-color:%s;width:%s;"></span>
				</div>',
				$color,
				$label,
				$percentage,
		...
		function shortcode($atts = array(), $post_type){
			...
			if( $posts ) {
				...
				$slider_id = ( 'slider' == $style )? 'id="team-slider-' . $this->instance.'"': '';
				$out .= ( 'slider' == $style)? $this->slideLoader : '';

				if ( ! is_singular( 'team' ) ) {
					$out .= '<div '.$slider_id.' class="loops-wrapper shortcode ' . $post_type  . ' ' . $style . ' '. $cpt_layout_class .'">';
				} else {
					$out .= '<div '.$slider_id.' class="shortcode ' . $post_type  . ' list-post">';
				}
					if ( 'slider' == $style ) {
						$option = 'setting-team_slider';
						$out .= sprintf('<div class="slideshow-wrap"><ul class="slideshow team-holder" data-id="team-slider-%s" data-autoplay="%s" data-effect="%s" data-speed="%s" data-visible="%s" data-scroll="%s" data-wrap="%s" data-slidernav="%s" data-pager="%s">',
							$this->instance,
